...
import GeneralLedgerHeader from "./components/account-report-header";
...
export default function AccountPayableClient() {
  ...
  const total = useMemo(() => {
    const totalInvoiceAmount = accountPayable?.reduce(
      (prev, acc) => prev + acc.invoiced_amount,
      0
    );
    const totalPaidAmount = accountPayable?.reduce(
  ...
  return (
    <div>
      <ListLayout title="Cuentas por Pagar">
        <Card>
          <CardHeader>
            ...
            <DataTable
              data={[
                ...(accountPayable || []),
                {
                  ...
                  currency:
                    accountPayable != undefined && accountPayable.length > 0
