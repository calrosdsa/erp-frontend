    ...
    return(
      ...
      {/* <AvatarImage src="https://github.com/shadcn.png" alt="@shadcn" /> */}
                        ...
                        <div className="flex flex-col w-full">
                            ...
                            <span className=" font-semibold">{fullName(item.profile_gn,item.profile_fn)}</span>
                            <span className=" text-xs">{formatLongDate(item.send_at,i18n.language)}</span>
                            </div>
                            <RenderMentionText
  ...
  sortedMentions.forEach((mention, index) => {
    ...
    segments.push(
      <Link
        ...
        to={route.toRoute({
            main:mention.entity_href,
            routeSufix:[mention.party_name],
            q:{
                id:mention.party_id
            }
        })}
        className="text-blue-700 rounded px-1"
      >
        {mention.party_name}
      </Link>
    );
